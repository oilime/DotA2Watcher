...
LEAGUE = os.path.expanduser('~/.Steam_watcher/league.json')
...
OPENDOTA_LEAGUES = 'https://api.opendota.com/api/leagues/{}'
OPENDOTA_LEAGUES_MATCHES = 'https://api.opendota.com/api/leagues/{}/matches'
...
sv_help = '''
[DotA2Watcher] 监视群友下分的机器人
'''.strip()

sv = Service(
    name='DotA2Watcher',  # 功能名
...
class Dota2:
    ...
    def get_last_match(id64):
        try:
            ...
            sv.logger.error('LAST_MATCH 网络延迟: {e}')
            return 0, 0
    ...
    def get_rank_tier(id3):
        try:
            ...
            sv.logger.error('OPENDOTA_PLAYERS 网络延迟: {e}')
            return '', 0
        except Exception as e:
            return '', 0
    ...
    def get_match(self, match_id):
        ...
        try:
            try:
                ...
                sv.logger.error('OPENDOTA_MATCHES 网络延迟: {e}')
                raise
                ...
                if steamdata['DOTA2_matches_pool'][match_id]['request_attempts'] >= MAX_ATTEMPTS:
                    ...
                    if not match.get('players'):
                        ...
                        try:
                            ...
                            sv.logger.error('MATCH_DETAILS 网络延迟: {e}')
                            raise
            ...
            sv.logger.error('{match_id}{attempts} {e}')
    ...
    def get_matches_report(self):
        ...
        for match_id, match_info in steamdata['DOTA2_matches_pool'].items():
            if match_info.get('is_solo'):
                ...
                if match:
                    if match.get('error'):
                        sv.logger.error('比赛编号{} 在分析结果中发现错误 {}'.format(match_id, match['error']))
            ...
            elif match_info.get('is_league'):
                match = self.get_match(match_id)
                if match:
                    if match.get('error'):
                        sv.logger.error('比赛编号{} 在分析结果中发现错误 {}'.format(match_id, match['error']))
                        m = '\n在分析联赛结果中发现错误 {}'.format(match['error'])
                    else:
                        self.generate_match_image(match_id)
                        m = '刚刚{}和{}在{}中的比赛结束了！'.format(match['radiant_team']['tag'], match['radiant_team']['tag'], match_info['league_name'])
                        m += '\n[CQ:image,file=file:///{}]'.format(os.path.join(DOTA2_MATCHES, f'{match_id}.png'))
                    reports.append(
                        {
                            'message': m,
                            'target_groups': match_info['groups'],
                            'user': [],
                        }
                    )
                    todelete.append(match_id)
...
async def init_fonts():
    ...
    try:
        ...
        if os.path.getsize(font_path) < 19647736:
            sv.logger.error('字体文件不完整')
            font_OK = False
    except Exception as e:
        sv.logger.error(f'初始化字体失败: {e}')
    if font_OK:
        return
    try:
        with open(font_path, 'wb') as f:
            ...
            for i in range(1, 193):
                ...
                sv.logger.info(f'正在重新下载字体({per:.2f}%)，预计还需要{t1:.2f}秒')
                f.write(requests.get(f'https://yubo65536.gitee.io/manager/assets/MSYH/x{n}', timeout=10).content)
        sv.logger.info('字体下载完成')
    except Exception as e:
        sv.logger.error('字体下载失败: {e}')


async def init_images():
    sv.logger.info('初始化图片')
    ...
    for img in images:
        ...
        # sv.logger.info('初始化图片({}/{})'.format(downloaded + successful + failed + 1, total))
        try:
        ...
        try:
            ...
            failed += 1
    sv.logger.info(f'从本地读取{successful}，重新下载{downloaded}，读取/下载失败{failed}')
...
async def get_news_async():
    ...
    try:
        ...
        sv.logger.error('PLAYER_SUMMARY 网络延迟: {e}')
        j = {'response': {'players': []}}
    for p in j['response']['players']:
        ...
        if steamdata['players'][id3]['last_DOTA2_action'] >= now - 10800 or datetime.now().minute == MINUTE:
            # print('{} 请求最近比赛更新 {}'.format(datetime.now(), id64))
            match_id, start_time = DotA2.get_last_match(id64)
        else:
        ...
        # 每3小时请求一次天梯段位
        if datetime.now().hour % 3 == 0 and datetime.now().minute == MINUTE:
            pname, cur_rank = DotA2.get_rank_tier(id3)
    ...
    news += DotA2.get_matches_report()

    if len(news) > 0:
        sv.logger.info(f'Steam雷达扫描到了{len(news)}个新事件')
...
@nonebot.on_startup
async def steam_init():
    sv.logger.info('初始化Steam 开始！')
    global DotA2, WhoIs, MINUTE
    ...
    if not os.path.exists(LEAGUE):
        dumpjson(DEFAULT_LEAGUE, LEAGUE)

    sv.logger.info('初始化Steam 完成！')


@sv.on_fullmatch('订阅steam')
async def steam_register(bot, ev: CQEvent):
    group_id = str(ev['group_id'])
    steamdata = loadjson(STEAM)
    if group_id in steamdata['subscribe_groups']:
        await bot.send(ev, '本群已订阅Steam')
    else:
        steamdata['subscribe_groups'].append(group_id)
        dumpjson(steamdata, STEAM)
        await bot.send(ev, '订阅Steam成功')


@sv.on_fullmatch('取消订阅steam')
async def steam_unregister(bot, ev: CQEvent):
    group_id = str(ev['group_id'])
    steamdata = loadjson(STEAM)
    if group_id in steamdata['subscribe_groups']:
        steamdata['subscribe_groups'].remove(group_id)
        dumpjson(steamdata, STEAM)
        await bot.send(ev, '取消订阅Steam成功')
    else:
        await bot.send(ev, '本群未订阅Steam')


@sv.on_rex('(怎么)?绑定 *steam(.*)', re.I)
async def register_help(bot, ev: CQEvent):
    group_id = str(ev['group_id'])
    ...
    try:
        if re_match[1]:
            ...
            return
        id3 = int(re_match[2])
        await bot.send(ev, f'正在尝试绑定并初始化玩家信息')
        ...
        steamdata = loadjson(STEAM)
        # 之前已经绑定过
        if steamdata['subscribers'].get(user_id):
            old_id3 = steamdata['subscribers'][user_id]
            if id3 == old_id3:
                success = f'你已绑定{old_id3}'
            if old_id3 != id3:
                steamdata['players'][old_id3]['subscribers'].remove(user_id)
                if not steamdata['players'][old_id3]['subscribers']:
                    del steamdata['players'][old_id3]
                success += f'\n已自动解除绑定{old_id3}'
        steamdata['subscribers'][user_id] = id3
        if steamdata['players'].get(id3):
            steamdata['players'][id3]['subscribers'].append(user_id)
            ...
            try:
                ...
                if match_id and start_time:
                    ...
                    s2 = '没有查询到最近的Dota 2比赛'
                name, rank = DotA2.dota2.get_rank_tier(id3)
                if rank:
                    s3 = '现在是{}{}'.format(
                        PLAYER_RANK[rank // 10], rank % 10 or '')
                else:
                    s3 = '没有查询到天梯段位'
                action = max(
                    start_time, now if gameextrainfo == 'Dota 2' else 0)
                success += '\n{}，{}，{}'.format(s1, s2, s3)
            except Exception as e:
                success += '\n初始化玩家信息失败'
                sv.logger.error('初始化玩家信息失败: {e}')
        ...
        dumpjson(steamdata, STEAM)
        await bot.send(ev, success.format(id3))
    except:
        await bot.send(ev, usage)

@sv.on_fullmatch('解除绑定steam')
async def steam_unbind(bot, ev: CQEvent):
    user_id = str(ev['user_id'])
    steamdata = loadjson(STEAM)
    if steamdata['subscribers'].get(user_id):
        id3 = steamdata['subscribers'][user_id]
        steamdata['players'][id3]['subscribers'].remove(user_id)
        if not steamdata['players'][id3]['subscribers']:
            del steamdata['players'][id3]
        del steamdata['subscribers'][user_id]
        dumpjson(steamdata, STEAM)
        await bot.send(ev, '解除绑定成功')
    else:
        await bot.send(ev, '没有找到你的绑定记录')


@sv.on_rex('(.+)在(干|做|搞|整)(嘛|啥|哈|什么)')
async def alias(bot, ev: CQEvent):
    group_id = str(ev['group_id'])
    user_id = str(ev['user_id'])
    match_msg = ev['match']
    name = match_msg.group(1).strip()
    steamdata = loadjson(STEAM)
    memberdata = loadjson(MEMBER)
    if re.search('群友', name):
        ...
        global WhoIs
        is_solo = True
        obj = WhoIs.object_explainer(group_id, user_id, name)
        name = obj['name'] or name
        steam_info = steamdata['players'].get(steamdata['subscribers'].get(obj['uid']))
        if steam_info:
            sids = steam_info.get('steam_id64')
            if not sids:
                await bot.send(ev, IDK)
                return
        else: # steam_info is None
            if obj['uid'] == UNKNOWN:
                await bot.send(ev, f'我们群里有{name}吗？')
                return
            await bot.send(ev, f'{IDK}，因为{name}还没有绑定SteamID')
            return
    replys = []
    try:
        j = requests.get(PLAYER_SUMMARY.format(APIKEY, sids), timeout=10).json()
    ...
    if replys:
        ...
        await bot.send(ev, '群友都没在玩游戏')
        return
    await bot.send(ev, IDK)

@sv.on_rex('查询(.+)的天梯段位$')
async def rank_query(bot, ev: CQEvent):
    group_id = str(ev['group_id'])
    user_id = str(ev['user_id'])
    await bot.send(ev, '正在查询...')
    match_msg = ev['match']
    name = match_msg.group(1).strip()
    steamdata = loadjson(STEAM)
    memberdata = loadjson(MEMBER)
    if re.search('群友', name):
        ...
        if not id3:
            if obj['uid'] == UNKNOWN:
                await bot.send(ev, f'我们群里有{name}吗？')
                return
            await bot.send(ev, f'查不了，{name}可能还没有绑定SteamID')
            return 
    ...
    for id3 in players_in_group:
        global DotA2
        name, rank = DotA2.get_rank_tier(id3)
        if rank:
...
@sv.on_rex('查询(.+)的(常用英雄|英雄池)$')
async def heroes_query(bot, ev: CQEvent):
    group_id = str(ev['group_id'])
    user_id = str(ev['user_id'])
    match_msg = ev['match']
    name = match_msg.group(1).strip()
    item = match_msg.group(2).strip()
    if name == '群友':
        await bot.send(ev, '我也妹这功能啊，只能一个一个查')
        return
    steamdata = loadjson(STEAM)
    global WhoIs
    obj = WhoIs.object_explainer(group_id, user_id, name)
    ...
    if item == '常用英雄':
        hero_stat = []
        if j[0]['games'] == 0:
            await bot.send(ev, f'这个{name}是不是什么都没玩过啊')
            return
        for i in range(5):
            if j[i]['games'] > 0:
                ...
                break
        await bot.send(ev, f'{name}玩得最多的{len(hero_stat)}个英雄：\n' + '\n'.join(hero_stat))
        return
    if item == '英雄池':
        hero_num = 0
        hero_ge20 = 0
        if j[0]['games'] == 0:
            await bot.send(ev, f'这个{name}什么都没玩过啊哪来的英雄池')
            return
...
async def heroes_query(bot, ev: CQEvent):
    ...
    match_msg = ev['match']
    try:        
        match_id = str(int(match_msg.group(1)))
        steamdata = loadjson(STEAM)
        if steamdata['DOTA2_matches_pool'].get(match_id, 0) != 0:
            await bot.send(ev, f'比赛{match_id}已在比赛缓冲池中，战报将稍后发出')
            return
        steamdata['DOTA2_matches_pool'][match_id] = {
            'request_attempts': 0,
            'players': [],
        ...
        dumpjson(steamdata, STEAM)
        await bot.send(ev, f'已将比赛{match_id}添加至比赛缓冲池，战报将稍后发出')
    except Exception as e:
        sv.logger.error(f'查询战报失败: {e}')
        await bot.send(ev, usage)

@sv.on_rex('查询(.+)的最近比赛$')
async def lastmatch_query(bot, ev: CQEvent):
    group_id = str(ev['group_id'])
    user_id = str(ev['user_id'])
    match_msg = ev['match']
    name = match_msg.group(1).strip()
    ...
    if not id3:
        ...
        return
    global DotA2
    match_id, start_time = DotA2.get_last_match(id3)
    if match_id and start_time:
        replys = []
        match_id = str(match_id)
        replys.append('查到了')
...
async def league_register(bot, ev: CQEvent):
    ...
    try:
        ...
        if leaguedata.get(league_id, 0) != 0:                    
            league_name = leaguedata[league_id]['league_alias'] if leaguedata[league_id]['league_alias'] else leaguedata[league_id]['league_name']
            if group_id in leaguedata[league_id]['subscribers']:
                await bot.send(ev, f'{league_name}已经订阅，无需重复订阅')
                return
            else:
                leaguedata[league_id]['subscribers'].append(group_id)
                dumpjson(leaguedata, LEAGUE)
                await bot.send(ev, f'已订阅{league_name}')
                return
        try:
            j = requests.get(OPENDOTA_LEAGUES.format(league_id), timeout=10).json()
        except requests.exceptions.RequestException as e:
            sv.logger.error('OPENDOTA_LEAGUES 网络延迟: {e}')
            await bot.send(ev, '网络延迟，请稍后再试')
            return
        league_name = j['name']
        try:
...
async def query():
    ...
    if not groups:
        return
    news = await get_news_async()
    sends = []
    for msg in news:
        for g in groups:
            if str(g) in msg['target_groups']:
                await sv.bot.send_private_msg(group_id=int(g), msg=msg['message'])
